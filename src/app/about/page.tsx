      ...
      <div className="max-w-6xl mx-auto space-y-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          <div className="w-full lg:w-1/2 relative">
            <div className="relative z-10 rounded-[3rem] overflow-hidden shadow-2xl bg-white border border-slate-100 p-2 max-w-md mx-auto lg:mx-0">
              <div className="rounded-[2.5rem] overflow-hidden relative bg-gradient-to-b from-slate-50 to-slate-100">
                <Image 
                  src="/doctor.png" 
            ...
            {/* Floating Badge */}
            <div className="absolute top-1/2 -right-4 md:right-0 bg-white p-4 rounded-2xl shadow-xl z-20 flex items-center gap-3 border border-slate-100 animate-bounce-slow">
              <ShieldCheck className="text-green-500" size={24} />
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Board Certified</p>
                <p className="font-bold text-slate-800 text-sm">Neurosurgeon</p>
              </div>
            </div>
          </div>
          
          <div className="w-full lg:w-1/2 space-y-8 text-center lg:text-left">
            <div className="inline-block bg-primary/10 text-primary px-4 py-2 rounded-full text-xs font-bold tracking-widest uppercase border border-primary/20">
              Chief Neurosurgeon
            </div>
        ...
        {/* Surgical Philosophy */}
        <div className="glass bg-white p-10 md:p-16 rounded-[3rem] border border-slate-100 shadow-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-secondary/10 rounded-full blur-3xl"></div>
          <div className="relative z-10 max-w-3xl space-y-6">
            <h2 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
              <Microscope className="text-primary" size={32} /> Surgical Philosophy
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8">
          ...
          <div className="md:col-span-2 space-y-8 glass p-8 md:p-10 rounded-[2.5rem] bg-white border border-slate-100 shadow-sm hover:shadow-xl transition-shadow duration-500">
            <div className="flex flex-col md:flex-row md:items-center gap-6 border-b border-slate-100 pb-6">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-accent/10 text-accent rounded-2xl"><Trophy size={28} /></div>
                <h2 className="text-2xl font-bold text-slate-900">Awards & Achievements</h2>
              </div>
              <p className="text-slate-500 text-sm md:ml-auto max-w-sm leading-relaxed">A track record of groundbreaking contributions to neurological literature and surgical excellence.</p>
            </div>
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <li key={idx} className="flex gap-4 items-start bg-slate-50 border border-slate-100 p-4 rounded-2xl hover:bg-slate-100 transition-colors cursor-default">
                  <BookOpen className="text-accent shrink-0 mt-1" size={18} />
                  <span className="text-slate-700 text-sm font-medium leading-relaxed">{achievement}</span>
                </li>
              ))}
            </ul>
          </div>
